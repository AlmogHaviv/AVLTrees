...
def main():
    test_is_valid_bst_given_valid_tree_returns_true()
    test_is_valid_bst_given_invalid_tree_returns_false()
    test_is_valid_avl_raises_error_for_unbalanced_tree()
    test_avl_node_given_child_returns_child()
    test_search_given_key_should_return_node_if_key_is_in_the_tree()
    test_to_array_given_tree_should_return_nodes_in_valid_order()
    test_insert_given_key_and_value_should_add_node_to_tree()
...
def test_successor_given_maximal_value_of_tree_should_return_null():
    tree = build_sample_avl()
    node = tree.search(24)
    assert tree.successor(node) is None
